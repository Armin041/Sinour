...
import { fetchProductNameApi } from "../productsNameSlice"
...
function ProductsNameList() {


    const fetchProductsNameList = () => {

        const response = dispatch(fetchProductNameApi())
        return response
    }
    ...
    useEffect(() => {
        fetchProductsNameList()
    }, [])
    const ProductsNameList = useSelector(state => state.productsName.ProductName)
    ...
    useEffect(() => {
        setProductsName(ProductsNameList)
        setFilteredProductName(ProductsNameList)

    }, [ProductsNameList])

    const removeFilter = () => {
        setFilteredProductName(productsName)

    }

    const applyFilter = (params) => {
        console.log(params)
        let filteredTransactions = filteredProductName.filter((t) => { return t.gender == params })
        setFilteredProductName(filteredTransactions)
    }

    // Search according to name
    const applySearch = (value) => {
        let filteredTransactions = filteredProductName.filter((t) => { return t.name.toLowerCase().includes(value.toLowerCase()) })
        setFilteredProductName(filteredTransactions)
    }
...
export default ProductsNameList